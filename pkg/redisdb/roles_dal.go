...
func NewRedisRolesDal() *redis_roles_dal {
	timeoutSeconds, err := strconv.Atoi(os.Getenv("REDIS_QUERY_TIMEOUT_SECONDS"))
	if err != nil {
		timeoutSeconds = 30
	}

	syncInterval, err := strconv.Atoi(os.Getenv("REDIS_SYNC_INTERVAL"))
	if err != nil {
		syncInterval = 10
	}
	...
	return &redis_roles_dal{
		...
		mongo:          connection.NewMongoStore(),
...
func (r *redis_roles_dal) InitialSync() error {
	...
	cursor, err := r.mongo.Database().Collection(r.collectionName).Find(ctx, bson.M{})
	if err != nil {
...
func (r *redis_roles_dal) SyncRolesCollection(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			cursor, err := r.mongo.Database().Collection(r.collectionName).Find(syncCtx, filter)
			if err != nil {
